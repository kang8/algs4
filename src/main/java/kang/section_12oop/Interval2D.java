...
public class Interval2D {
    ...
    public boolean contains(Interval2D that) {
        return this.contains(new Point2D(that.x.min(), that.y.min()))
                && this.contains(new Point2D(that.x.min(), that.y.max()))
                && this.contains(new Point2D(that.x.max(), that.y.min()))
                && this.contains(new Point2D(that.x.max(), that.y.max()));
    }
